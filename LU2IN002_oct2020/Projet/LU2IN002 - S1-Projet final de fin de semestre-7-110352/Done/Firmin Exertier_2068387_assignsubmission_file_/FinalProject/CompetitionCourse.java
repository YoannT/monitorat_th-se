...
class CompetitionCourse extends Competition{
	private ArrayList<Coureur> qualifies;
	
	public CompetitionCourse(String pDate){
		super(pDate,"CompetitionCourse");
		this.participant=new ArrayList<Coureur>();
		this.qualifies=new ArrayList<Coureur>();
	}
	
	public String toString(){
		return "Competition de course du "+this.date;
	}
	...
	public void courseListe(ArrayList<Coureur> liste){
		for(int i=0;i<liste.size();i++){
			Course course=new Course(this.date,liste.get(i));
			course.setTemps(liste.get(i).getScore());
			liste.get(i).setScore(course.getTemps());	
		}
				
	}
	
	public ArrayList<Coureur> organiserResultats(ArrayList<Coureur> liste){
		ArrayList<Coureur> retour=new ArrayList<Coureur>(liste);
		boolean fait=false;
		Coureur temp=null;
		while(!fait){
			fait=true;		
